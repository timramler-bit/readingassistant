 ...
 * Browsers can sometimes crash if process.env is accessed directly when not defined.
 */
const getApiKey = () => {
  try {
...
export const analyzePassage = async (passage: string, language: 'English' | 'Japanese') => {
  ...
  if (!apiKey) {
    return { grammarPoints: [], vocabulary: [], summary: "AI analysis requires an API key. Please check your configuration." };
  }
...
export const getWordInsight = async (word: string, fullPassage: string, language: 'English' | 'Japanese') => {
  const apiKey = getApiKey();
  if (!apiKey) return { meaning: "API Key Missing", pronunciation: "N/A" };
